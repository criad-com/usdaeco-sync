...
from aeco_sync.profiles import apply_profile, load_profile
...
def test_validator_accepts_profile(tmp_path):
    from aeco_sync import register_plugins, validators
    from pxr import Usd
    register_plugins()
    ...
    path.write_text('{"include_builtin": false, "severity_overrides": {"bindingMissing": "info"}}')
    issues = validators.validate_stage(stage, profile=path)
    matching = [e for e in issues if e.GetName() == "bindingMissing"]
